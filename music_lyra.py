...
def _parse_address(addr: str):
    """Parse a lava host string of the form ``scheme://host:port``.
    The scheme may be ``http`` or ``https``.  The port is optional and
    defaults to 6761.  Returns ``(host, port, secure)`` where secure is a
    bool derived from the scheme (``https`` -> ``True``).
    """
    m = re.match(r"^(?P<scheme>https?)://(?P<host>[^:/]+)(?::(?P<port>\d+))?", addr)
    scheme = m.group("scheme")
    host = m.group("host")
    port = int(m.group("port")) if m.group("port") else 6761
    secure = scheme == "https"
...
def music_now_playing_embed(bot: commands.Context, track: lava_lyra.Track):
...
def requester_string(bot: commands.Bot, track: lava_lyra.Track):
    if track.requester: return f"<@{track.requester.id}>"
...
def search_embed(arg: str, result: list[lava_lyra.Track], index: int) -> discord.Embed:
    embed = discord.Embed(title=f"🔍 Search results: `{result.name if isinstance(result, lava_lyra.Playlist) else arg}`",
...
class SearchView(discord.ui.View):
    def __init__(self, bot: commands.Bot, ctx: commands.Context, arg: str, result: list[lava_lyra.Track], index: int):
...
class nextPage(discord.ui.Button):
    def __init__(self, bot: commands.Bot, ctx: commands.Context, arg: str, result: list[lava_lyra.Track], index: int, l: str):
...
class SelectChoice(discord.ui.Select):
    def __init__(self, bot: commands.Bot, ctx: commands.Context | discord.Interaction, index: int, result: list[lava_lyra.Track]):
...
async def voice_channel_connector(ctx: commands.Context | discord.Interaction):
    ...
    # Get a healthy node from the pool that belongs to this bot
    node = pool.get_best_node(algorithm=lava_lyra.NodeAlgorithm.by_health)
    vc = await member.voice.channel.connect(cls=NoobGPTPlayer, self_deaf=True, node=node)
    return vc
...
async def setup(bot: commands.Bot):
    await bot.add_cog(MusicUtil2(bot))