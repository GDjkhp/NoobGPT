...
from util_database import set_ai_mode, get_database2
...
async def ai_respond_mode(ctx: commands.Context, model: str):
    ...
    if model != "off": await ctx.reply(f"{model} has been set as my default response mode. talk to me and see what happens.")
    else: await ctx.reply(f"ai response mode has been disabled. talk to me and i'll roast you instead.")
...
class AIModeCog(commands.Cog):
    ...
    @commands.hybrid_command(description="set ai mode description placeholder")
